...
def train(
    ...
    colorizer = ImageColorizer(output_folder1="E:/Python_yhgtProject/Color2Embed-main/data/train_color_l",  # 亮度图像
                               output_folder2="E:/Python_Project/Color2Embed-main/data/train_img/images/1"  # 上色图像
                               )
    # 创建图像着色器
    processor = ImageProcessor(args.ckpt1, args.root_dir, args.pred_dir, args.size1, (256, 256))
    ...
    for idx in pbar:
        ...
        img, img_ref, img_lab = next(loader)  # 这里读入的图片的像素值是0到255的整数，而不是0到1的浮点数。这里的img是输入图像，img_ref是参考图像，img_lab是目标的Lab色彩空间表示。
        # print(img.shape, img_ref.shape, img_lab.shape)
        # 创建图像处理器

        # 将参考图像保存到文件夹便于共同目标检测
        img_ref_np = []

        for j in range(img_ref.shape[0]):
            img_ref_np = img_ref[j].permute(1, 2, 0).detach().cpu().numpy()  # 先进行转置，再转换为numpy数组
            img_ref_np = (img_ref_np * 255).astype(np.uint8)  # 值缩放到 [0, 255] 并且转换为 uint8 类型
            save_dir = "E:/Python_Project/Color2Embed-main/data/train_img/references/1"
            os.makedirs(save_dir, exist_ok=True)  # 确保路径存在
            save_path = os.path.join(save_dir, f"{j}.jpg")
            # 使用OpenCV保存图像
            cv2.imwrite(save_path, img_ref_np)
        ...
        # 进行图像处理
        rm_result_tensor = processor.process_images( )  # 这里的result_tensor是黑白的掩膜图像的张量，用它进行上色
        # print(result_tensor.size())  # 打印张量大小
        # processor.display_images(result_tensor)
        # 我们希望经过预训练的上色模型将灰度图上色后，根据参考图像的色彩信息，对彩色图像进行补充上色。
        rm_result_tensor = rm_result_tensor.to(device)
        ...
        mask = rm_result_tensor[:, 0:1, :, :]  # 提取掩膜图像的第一个通道，这个通道的值范围是[0,1]，表示像素的透明度。黑色的像素值为0，白色的像素值为1。
        fused_ab = pixel_attention(fake_swap_ab, pretrained_ab, mask)
        ...
        if get_rank() == 0:  # 如果当前进程的 rank 是 0，那么就更新进度条的描述信息。
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    # 指定你的目录路径
                    directories = [
                        "E:/Python_Project/Color2Embed-main/data/train_img/images/1",
                        "E:/Python_Project/Color2Embed-main/data/train_img/references/1",
                        "E:/Python_yhgtProject/Color2Embed-main/data/train_color_l"
                    ]
                    ...
                    for inum in range(10):
                        #对"E:/Python_Project/Color2Embed-main/data/train_img/images/1"和"E:/Python_Project/Color2Embed-main/data/train_img/references/1"两个文件夹内的图片清空的代码
                        ...
                        #
                        #将val_ref_path的图片copy到save_dir中的代码：

                        save_dir = "E:/Python_Project/Color2Embed-main/data/train_img/references/1"
                        # 确保保存目录存在
                        os.makedirs(save_dir, exist_ok=True)
                        # 创建保存路径
                        save_path = os.path.join(save_dir, f"{inum}.jpg")

                        # 使用shutil.copy复制图片
                        shutil.copy(val_ref_path, save_path)
                        ...
                        rm_result_tensor = rm_result_tensor.to(device)
                        mask = rm_result_tensor[:, 0:1, :, :]  # 提取掩膜图像的第一个通道，这个通道的值范围是[0,1]，表示像素的透明度。黑色的像素值为0，白色的像素值为1。
                        val_fused_ab = pixel_attention(fake_swap_ab, pretrained_ab,mask)
                        val_fused_ab_first_image = val_fused_ab[0].unsqueeze(0)  # 这里的维度是
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='GCoNet', type=str, help="Model name")
    parser.add_argument('--size1', default=256, type=int, help='Size for image processing')
    parser.add_argument('--ckpt1', default='E:/Python_Project/Color2Embed-main/GCoNet/GCoNet.pth', type=str,
                        help='Model checkpoint path')
    parser.add_argument('--root_dir', default='E:/Python_Project/Color2Embed-main/data/train_img', type=str,
                        help='Root directory of images and references')
