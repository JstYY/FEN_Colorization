...
#         fused_output_ab = model_output_ab * (1 - attention_weights) + pretrained_output_ab * attention_weights
#
#         return fused_output_ab
class PixelAttention(nn.Module):
    def __init__(self):
    ...
    def forward(self, model_output_ab, pretrained_output_ab, masks_tensor):
        # 重复张量以匹配ab通道的数量
        mask_repeat = masks_tensor.repeat(1, 2, 1, 1)

        # 根据掩膜权重直接混合两个ab通道的输出

        # fused_output_ab = model_output_ab * mask_repeat + pretrained_output_ab * (1 - mask_repeat)
        fused_output_ab = model_output_ab * mask_repeat + pretrained_output_ab * (1 - mask_repeat)
