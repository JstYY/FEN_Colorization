...
def RGB2Lab(inputs):
    # 首先确保输入在CPU上，并且是一个NumPy数组
    if isinstance(inputs, torch.Tensor):
        if inputs.is_cuda:
            inputs = inputs.cpu()  # 如果在CUDA上，首先移动到CPU
        inputs = inputs.numpy()   # 然后转换为NumPy数组
    # 然后执行rgb到lab的转换
...
if __name__ == "__main__":
    ...
    model_name = 'Color2Embed_C_5.5w_C'
    ckpt_path = 'E:/Python_Project/Color2Embed-main/experiments/default/055000.pt'
    ...
    for i in range(imgs_num):
        ...
        out_img_path = os.path.join(out_dir_path, '%03d_ref_in.png'%idx)
        ...
        mask_tensor = processor.process_images(img_dd, img2_lab)

        img_dd_lab = rgb_to_lab_tensor(img_dd)  # 获取转换后的Lab图像张量
        # 上色图像并保存到文件夹
        img1 = img1.to(device)#
        ...
        with torch.no_grad():
            ...
            #输出张量的最大值和最小值
            # print(torch.max(fake_ab), torch.min(fake_ab))
            #
            # print(torch.max(pretrained_ab), torch.min(pretrained_ab))

            # print(pretrained_ab.shape) # torch.Size([1, 2, 256, 256])

            # 进行插值以匹配fake_ab大小
            pretrained_ab_resized = F.interpolate(pretrained_ab , (imgsize, imgsize), mode='bilinear',
                                                  recompute_scale_factor=False, align_corners=False)

            # 将预训练模型的输出移动到GPU上 (确保fake_swap_ab已经在这里定义了)
            pretrained_ab_resized = pretrained_ab_resized.to(fake_ab.device)
            # 注意力机制融合ab通道
            #将fake_ab，pretrained_ab_resized变为数组，然后展示图像


            # print(fake_ab.shape, pretrained_ab_resized.shape,mask_tensor.shape)

            fused_ab = pixel_attention(fake_ab * 110, pretrained_ab_resized * 110,mask_tensor)  # 融合
            fused_ab = F.interpolate(fused_ab / 110, size=(height, width), mode='bilinear', align_corners=False)
            # 将 img1_lab 重复扩展到与 fused_ab 批次大小一致

            # 将L通道与融合后的ab通道结合，并进行Lab到RGB的转换
            # print(fused_ab.shape, img1_lab.shape)
            fake_img_lab = torch.cat((img1_lab[:, :1, :, :], fused_ab*110), 1)
            fake_img_rgb = Lab2RGB_out(fake_img_lab)

            # 保存生成的图像
            io.imsave(out_img_path, fake_img_rgb)  # 这里的fake_img是RGB图像

        #     re_img, re_img_lab = preprocessing(fake_img_lab)#这里在将RGB图像转化为Lab图像
