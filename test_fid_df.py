...
def load_images(path, target_size=None, gray=False):
    ...
    Warnings: Expects all images to be resized to a target_size.
    Args:
        path: relative path to directory
        target_size: tuple of ints (W, H), represents the target width and height.
    Returns:
        final_images: np.array of (N, H_target, W_target, 3), dtype: np.uint8
    """
    image_paths = []
    ...
    if not image_paths:
        raise RuntimeError('No images found at the path provided:', path)

    if target_size is None:
        target_size = cv2.imread(image_paths[0]).shape[:2]  # Use the size of the first image as the target size.
    ...
    for impath in image_paths:
        ...
        im = cv2.resize(im, (target_size[1], target_size[0]))  # Make sure all images are resized to target size
...
def get_fid_for_batches(path1, path2, batch_size, use_multiprocessing=True):
    images1 = load_images(path1)  # 加载第一个目录的所有图像
    images2 = load_images(path2)  # 加载第二个目录的所有图像

    # 计算每一批次的FID分数
    n_batches = min(len(images1), len(images2)) // batch_size
    fid_scores = []
    for i in range(n_batches):
        # 获取当前批次的图像
        batch_images1 = images1[i * batch_size: (i + 1) * batch_size]
        batch_images2 = images2[i * batch_size: (i + 1) * batch_size]

        # 预处理图像
        batch_images1 = preprocess_images(batch_images1, use_multiprocessing)
        batch_images2 = preprocess_images(batch_images2, use_multiprocessing)

        # 计算当前批次的激活统计数据
        mu1, sigma1 = calculate_activation_statistics(batch_images1, batch_size)
        mu2, sigma2 = calculate_activation_statistics(batch_images2, batch_size)

        # 计算并存储当前批次的FID分数
        fid = calculate_frechet_distance(mu1, sigma1, mu2, sigma2)
        fid_scores.append(fid)
        print(f"FID for batch {i}: {fid}")

    # 计算所有批次FID分数的平均值
    avg_fid = np.mean(fid_scores)
    return fid_scores, avg_fid
...
if __name__ == "__main__":
    ...
    parser.add_option("--p1", "--path1", dest="path1",
                      default='E:\Python_Project\Color2Embed-main\\results\gray2color\exemplar_based\Color2Embed_IMGNET_5.5w',
                      help="Path to directory containing the real images")
    parser.add_option("--p2", "--path2", dest="path2",
                      default='E:\Python_Project\DDColor-master\data_list\ILSVRC2012_img_val_color',
    ...
    fids, avg_fid = get_fid_for_batches(options.path1, options.path2, options.batch_size, options.use_multiprocessing)
    print(f"FID scores for each batch: {fids}")
    print(f"Average FID score: {avg_fid}")